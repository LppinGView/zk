...
### 1.关于Zookeeper Atomic Broadcast zab协议
...
1.reliable delivery 可靠性投递: 消息m被一个服务投递,则最终被所有法定成员投递;

2.total order 整体有序: 消息a在一个服务中,在消息b之前被投递,则在每个服务中投递b之前,一定会先投递a;

3.causal order 因果有序: 如果由于消息a产生了消息b,且这两个消息都要被投递,则消息a必须先于b进行投递;

4.prefix property 前置条件: 如果消息m是 leader节点L 的最新的投递消息,则任何在m之前的所有消息,必须都已经被L投递;


[基于以上的必要条件,就可以构建正确的zookeeper数据库副本]

条件1和条件2 保证 所有的副本具有一致性视图;  [为啥? 1保证节点互通 2保证整体消息有序]
...
1.消息a和消息b 被同一个服务发送,并且消息a 在消息b之前 发起提议, 则a是b的因;

2.zab假定,任何时候都只有一个leader提交议题.如果一个leader改变,任何之前leader的commit提议都是新leader commit提议消息的因;
...
1.提议会产生冲突；paxos使用投票机制，去检测和解决冲突提议。

2.不能知道一个提议是否已经提交，进程必须能够辨别出提议是否提交。
...
# FastLeaderElection 使用tcp进行服务间选举，构造函数有两个参数(当期成员对象QuorumPeer，连接管理器QuorumCnxManager)
...
1.QuorumCnxManager构造函数
	初始化连接
2.initializeConnectionExecutor
...
Zookeeper 客户端API获取连接之后的操作入口类
	1.WatchRegistration 在命令发送时注册监听器，之后通过连接向服务端发送该监听器
	2.ClientWatchManager materialize物化方法 客户端从服务端接收已被触发的监听器，从而处理后置处理事件
